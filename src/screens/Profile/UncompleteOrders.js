...
function UncompleteOrders() {
  ...
  function removeItemFromCart(order_id) {
    setOrderArr(orderarr => {
      const updatedCart = orderarr.filter(item => item.orders_table.id !== order_id);
      return updatedCart;
    });
  }
              ...
              >
                ...
                <View style={{ flexDirection: "row", margin: 5, maxWidth: RFPercentage(36) }}>
                  <View>
                    ...
                    >
                      الكميه:<Text style={{ color: COLORS.gray_dark }}>{item.orders_table.amount_oil_order} </Text>
                    </Text>
                    <Text
                      style={{
                        fontFamily: FONT.font_Almarai_Bold,
                        color: COLORS.black,
                        fontSize: RFValue(14, h, w),
                        maxWidth: RFPercentage(25),
                      }}
                    >
                      تاريخ الطلب:<Text style={{ color: COLORS.gray_dark }}>{item.orders_table.time_date} </Text>
                    </Text>
                    <Text
                      style={{
                        fontFamily: FONT.font_Almarai_Bold,
                    ...
                    >
                      معاد الاستلام :<Text style={{ color: COLORS.gray_dark }}>{item.orders_table.time_date_recive} </Text>
                    </Text>
                    <Text
                      style={{
                        fontFamily: FONT.font_Almarai_Bold,
                    ...
                    >
                      مكان الاستلام :<Text style={{ color: COLORS.gray_dark }}>{item.orders_table.place_recieve} </Text>
                    </Text>
                  </View>
                  ...
                  <View style={{ alignItems: "center" }}>
                    ...
                    <TouchableOpacity style={{ flexDirection: "row" }}>
                      <Icon name="pen" size={hp(2)} style={{ color: COLORS.gray_dark, margin: 4 }} />
                      <Text
                        style={{
                          fontFamily: FONT.font_Almarai_Bold,
                    ...
                    <TouchableOpacity onPress={() => removeItemFromCart(item.orders_table.id)} style={{ flexDirection: "row" }}>
                      <Icon name="trash-alt" size={hp(2)} style={{ color: COLORS.red_logout, margin: 4 }} />
                      <Text
                        style={{
                          fontFamily: FONT.font_Almarai_Bold,
